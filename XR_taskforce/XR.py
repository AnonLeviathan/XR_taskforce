...
def send_document(doc_tosent):
    print('send doc is used for: ' + doc_tosent)
...
class Monitoring():
    ...
    def create_servant(self):
        ...
        shutil.copyfile("temp.json", fullname)
...
class Communication():
    def __init__(self, targetinfo):
        self.targetinfo = targetinfo
        self.server = xmlrpclibclient.ServerProxy("http://localhost:8000")
    ...
    def actif(self):
        if self.json_data['servant']:
            ...
            while True:
                ...
                send_document("update_log.txt")
                # time.sleep(3600) # send log every hours
                time.sleep(60)
        else:
            Fullsystem.read_json(self)
...
if __name__ == '__main__':
    targetinfo = Targetinfo()
    communication = Communication(targetinfo)
    communication.receive_msg()
